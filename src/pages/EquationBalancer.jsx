  ...
  return (
    <div className="flex flex-col items-center gap-4 select-none w-full">
      ...
      <div className="relative w-full max-w-[260px]">
        <div className="w-full h-3 bg-gradient-to-r from-slate-400 via-slate-200 to-slate-400 rounded-full relative"
          style={{ transform: 'rotate(-8deg)' }}>
          <div className="absolute left-[8%] bottom-full mb-0 w-16 h-12 bg-gradient-to-br from-slate-600 to-slate-900 border border-slate-400 rounded-t-lg flex items-center justify-center text-sm font-black text-white">
            6 + 8
          </div>
          <div className="absolute right-[8%] bottom-full mb-0 w-16 h-12 border-2 border-dashed border-blue-400/50 rounded-t-lg flex items-center justify-center text-blue-300 font-black text-lg">
            ?
          </div>
        </div>
        <div className="w-12 h-16 bg-gradient-to-br from-slate-600 to-slate-800 absolute left-1/2 -translate-x-1/2 -top-0.5 z-0"
          style={{ clipPath: 'polygon(50% 0%, 0% 100%, 100% 100%)' }} />
      </div>
      <div className="flex gap-3 mt-4">
        {[14, 8, 11].map((n, i) => (
          <div key={i} className="w-12 h-12 bg-gradient-to-b from-slate-500 to-slate-800 text-white font-black text-xl flex items-center justify-center"
            style={{ clipPath: 'polygon(20% 0%, 80% 0%, 100% 20%, 100% 100%, 0% 100%, 0% 20%)' }}>
            {n}
          </div>
        ))}
      </div>
      <p className="text-sm text-slate-400 font-medium">Pick the weight that balances!</p>
    </div>
  );
}
  ...
  if (!started) {
    return (
      <GameStartScreen
        ...
        emoji="⚖️"
        category="Algebra"
        description="The left side of the scale shows a sum. The right side has a known part and a missing weight. Pick the iron weight that perfectly balances the equation!"
        stats={[
          { label: 'XP each', value: '25' },
          { label: 'Style', value: 'Endless' },
